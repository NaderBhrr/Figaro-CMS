...
const Modal: FC<iProps> = ({
  ...
  const _onChange = (e: any): any => {
    ...
    if (e.target.name === "order") {
      if (Number(e.target.value) <= 0) {
        e.target.value =
          e.target.value === "0" ? "1" : Math.abs(e.target.value);
    ...
    return (
      <div>
        <label htmlFor="enumeration">
          {t("Enumeration")}{" "}
          {required.enumeration && <Badge danger>{t("Required")}</Badge>}
        </label>
        <Select
          name="enumeration"
  ...
  const renderReference = () => {
    ...
    const selectOptions: any = models
      .filter(
        (model: any) =>
          model.modelName !== "Asset" && model.identifier !== currentModel
      )
      .map((option: any) => ({
    ...
    return (
      <div>
        <label htmlFor="reference">
          {t("Reference")}{" "}
          {required.reference && <Badge danger>{t("Required")}</Badge>}
        </label>
